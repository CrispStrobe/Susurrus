...
def main():
    ...
    parser.add_argument(
        "--backend", "-b", default="crispasr", help="Transcription backend (default: crispasr)"
    )
    parser.add_argument("--model", "-m", default=None, help="Model path or HF model ID")
    ...
    # CrispASR-specific
    parser.add_argument(
        "--crispasr-backend", default=None, help="Force CrispASR sub-backend (parakeet, qwen3, ...)"
    )
    parser.add_argument("--vad", action="store_true", help="Enable VAD (CrispASR)")
    parser.add_argument(
        "--split-on-punct", action="store_true", help="Split at punctuation (CrispASR)"
    )
    ...
    backend = BackendClass(
        model_id=args.model, device=args.device, language=args.language, **kwargs
    )
