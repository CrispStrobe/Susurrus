...
class CTranslate2Backend(TranscriptionBackend):
    ...
    def transcribe(self, audio_path):
        ...
        try:
            ...
            raise ImportError(
                "ctranslate2, transformers, and librosa required for ctranslate2 backend"
            )
        ...
        preprocessor_missing = not all(
            os.path.exists(os.path.join(self.preprocessor_path, f)) for f in preprocessor_files
        )
        ...
        if preprocessor_missing:
            ...
            if not self.original_model_id:
                logging.error(
                    "Original model ID not specified. Cannot load tokenizer and processor."
                )
