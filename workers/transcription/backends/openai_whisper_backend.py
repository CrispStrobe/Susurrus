...
class OpenAIWhisperBackend(TranscriptionBackend):
    ...
    def __init__(
        self, model_id, device, language=None, word_timestamps=False, max_chunk_length=0, **kwargs
    ):
        ...
        if self.max_chunk_length > 0:
            ...
            for chunk_index, chunk in enumerate(merged_chunks):
                ...
                result = model.transcribe(
                    chunk_file.name, language=self.language, word_timestamps=self.word_timestamps
                )
            ...
            result = model.transcribe(
                audio_path, language=self.language, word_timestamps=self.word_timestamps
            )
