...
from utils.audio_utils import convert_audio_to_wav, is_valid_time, trim_audio
    ...
    if backend in device_fallbacks:
        for unsupported, fallback in device_fallbacks[backend]:
            if device.lower() == unsupported.lower():
                logging.warning(
                    f"Device '{device}' is not supported by backend '{backend}'. Switching to '{fallback}'."
                )
                return fallback
    return device
...
def main():
    ...
    try:
        ...
        parser.add_argument(
            "--device", type=str, default="auto", help="Device to use (auto, cpu, gpu, mps)"
        )
        parser.add_argument("--pipeline-type", default="default", help="Pipeline type")
        parser.add_argument(
            "--max-chunk-length", type=float, default=0.0, help="Max chunk length in seconds"
        )
        parser.add_argument("--output-format", default="txt", help="Output format for whisper.cpp")
        parser.add_argument(
            "--quantization", default=None, help="Quantization type for ctranslate2"
        )
        parser.add_argument(
            "--preprocessor-path", type=str, default="", help="Path to preprocessor files"
        )
        parser.add_argument("--original-model-id", type=str, default="", help="Original model ID")
        parser.add_argument(
            "--start-time", type=str, default=None, help="Start time for audio trimming in seconds"
        )
        parser.add_argument(
            "--end-time", type=str, default=None, help="End time for audio trimming in seconds"
        )
        ...
        parser.add_argument(
            "--ffmpeg-path", type=str, default="ffmpeg", help="Path to ffmpeg executable"
        )
        parser.add_argument(
            "--mistral-api-key",
            type=str,
        ...
        if audio_input and os.path.exists(audio_input):
            ...
            original_audio_path = download_audio(
                audio_url, proxies=proxies, ffmpeg_path=args.ffmpeg_path
            )
        ...
        if backend == "mlx-whisper":
            try:
                ...
                raise ImportError(
                    "mlx_whisper and huggingface_hub packages are required for mlx-whisper backend"
                )
                ...
                try:
                    ...
                    if word_timestamps and segment.words:
                        for word in segment.words:
                            ...
                            word_end = word.end
                            print(
                                f"  [{word_start:.3f} --> {word_end:.3f}] {word_text}", flush=True
                            )
                except Exception as e:
            ...
            segments = voxtral_api.transcribe_with_chunking(
                audio_input, language=language, max_duration=600  # 10 minutes per chunk
            )
            ...
            try:
                ...
                cmd = [
                    ...
                    "24",
                    "--timestamp",
                    "chunk" if not word_timestamps else "word",
                    "--transcript-path",
                    output_json,
                ]
                ...
                raise ImportError(
                    "insanely-fast-whisper not found. Install with: pip install insanely-fast-whisper"
                )
            except Exception as e:
            ...
            try:
                ...
                logging.info(
                    f"Using model file: {os.path.basename(model_path)} ({model_size:.2f} MB)"
                )
            except Exception as e:
            ...
            cmd = [
                ...
                audio_input,
                "-t",
                str(min(os.cpu_count() or 4, 8)),  # Reasonable thread count
            ]
            ...
            try:
                ...
                raise ImportError(
                    "ctranslate2, transformers, and librosa are required for ctranslate2 backend"
                )
            ...
            preprocessor_missing = not all(
                os.path.exists(os.path.join(preprocessor_path, f)) for f in preprocessor_files
            )
            ...
            if preprocessor_missing:
                ...
                if original_model_id is None:
                    logging.error(
                        "Original model ID is not specified. Cannot load tokenizer and processor."
                    )
            ...
            try:
                ...
                if not language:
                    ...
                    logging.info(
                        f"Detected language: {detected_language} with probability {probability:.4f}"
                    )
            ...
            try:
                ...
                raise ImportError(
                    "whisper-jax, jax, and jaxlib are required for whisper-jax backend"
                )
            ...
            try:
                ...
                raise ImportError(
                    "openai-whisper and pydub are required for OpenAI Whisper backend"
                )
            ...
            if max_chunk_length > 0:
                ...
                for chunk_index, chunk in enumerate(merged_chunks):
                    ...
                    result = model.transcribe(
                        chunk_file.name, language=language, word_timestamps=word_timestamps
                    )
                    ...
                    for segment in result["segments"]:
                        ...
                        if word_timestamps and "words" in segment:
                            for word in segment["words"]:
                                ...
                                print(
                                    f"  [{word_start:.3f} --> {word_end:.3f}] {word_text}",
                                    flush=True,
                                )
                ...
                result = model.transcribe(
                    audio_input, language=language, word_timestamps=word_timestamps
                )
