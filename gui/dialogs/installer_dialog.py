...
class DependencyInstallThread(QThread):
    ...
    def __init__(self, install_type="youtube"):
        super().__init__()
    ...
    def run(self):
        ...
        try:
            if self.install_type == "youtube":
                self._install_youtube_deps()
    ...
    def _install_youtube_deps(self):
        ...
        result = subprocess.run(
            [sys.executable, "-m", "pip", "install", "-U", "pytube"], capture_output=True, text=True
        )
        if result.returncode != 0:
...
class InstallerDialog(QDialog):
    ...
    def __init__(self, parent=None):
        ...
        # YouTube dependencies button
        youtube_btn = QPushButton("Install YouTube Dependencies")
        youtube_btn.clicked.connect(self.install_youtube)
    ...
    def install_youtube(self):
        ...
        self._run_installer_thread(
            "youtube",
            "Installing YouTube Dependencies",
            "Installing dependencies for YouTube downloading:\n"
            "• yt-dlp\n"
            "• pytube\n"
            "• pydub",
        )
