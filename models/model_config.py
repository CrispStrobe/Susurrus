    ...
    def find_or_convert_model(model_id, parent_widget=None):
        ...
        if os.path.exists(local_model_bin_path) and os.path.getsize(local_model_bin_path) > 0:
            if os.path.islink(local_model_bin_path):
                ...
                logging.info(
                    f"Replaced symlink with actual file for model.bin in: {local_model_dir}"
                )
            else:
        ...
        try:
            ...
            if "model.bin" in model_files:
                logging.info(
                    f"Found pre-converted CTranslate2 model in Hugging Face repo: {model_id}"
                )
                ...
                for file in [
                    ...
                    if file in model_files:
                        file_path = hf_hub_download(
                            repo_id=model_id, filename=file, local_dir=local_model_dir
                        )
        ...
        logging.info(
            f"Model not found in CTranslate2 format or couldn't be downloaded. Converting {model_id} to CTranslate2 format..."
        )
        ...
        if not ok_pressed:
            QMessageBox.information(
                parent_widget, "Information", "Quantization selection cancelled by user."
            )
        ...
        if reply == QMessageBox.StandardButton.No:
            QMessageBox.information(
                parent_widget, "Information", "Model conversion cancelled by user."
            )
        ...
        try:
            CTranslate2ModelConverter.convert_model(
                model_id, local_model_dir, quantization, parent_widget
            )
            logging.info(f"Model converted and saved to: {local_model_dir}")
        except Exception as e:
            logging.error(f"Error during model conversion: {e}")
            QMessageBox.critical(parent_widget, "Error", f"Error during model conversion: {str(e)}")
            return None, None
        ...
        try:
            ...
            QMessageBox.critical(
                parent_widget, "Error", f"Error downloading preprocessor files: {str(e)}"
            )
            return None, None
