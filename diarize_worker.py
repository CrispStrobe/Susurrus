...
class TranscriptionProgress:
    ...
    def _progress_indicator(self):
        ...
        while self.running and self.completed < self.total:
            ...
            if self.total > 0:
                ...
                print(f"\r[{mins:02d}:{secs:02d}] Transcribing: [{bar}] {percentage:.1f}% ({self.completed}/{self.total}) Est. remaining: {rem_mins}m {rem_secs}s", 
                      end="", flush=True)
            else:
    ...
    def finish(self):
        ...
        if self.total > 0:
            ...
            print(f"✅ Transcription completed in {mins:02d}:{secs:02d} ({avg_time:.2f}s per segment)")
        else:
...
def main():
    ...
    parser.add_argument('--device', type=str, default=None, help='Device to use (auto, cpu, cuda, mps)')
    parser.add_argument('--output-dir', type=str, default=None, help='Output directory for diarized segments')
    parser.add_argument('--diarization-model', type=str, default="Default", 
    ...
    parser.add_argument('--model-id', type=str, default='base', help='Whisper model ID to use for transcription')
    parser.add_argument('--language', default=None, help='Language code for transcription')
    parser.add_argument('--backend', type=str, default='transformers', help='Backend to use for transcription')
    
    # Output format parameters
    parser.add_argument('--output-formats', type=str, default='txt,srt,vtt', help='Output formats (comma separated)')
    parser.add_argument('--output-base', type=str, default=None, help='Base path for output files (without extension)')
    
    # Authentication verification
    parser.add_argument('--verify-auth', action='store_true', help='Verify Hugging Face authentication before processing')
    ...
    try:
        from diarize_audio import DiarizationManager, verify_authentication, EnhancedProgress
    ...
    if not output_base:
        output_base = os.path.join(output_dir, os.path.splitext(os.path.basename(args.audio_input))[0])
    ...
    if args.diarization_model not in available_models:
        ...
        logging.warning(f"Invalid diarization model: {args.diarization_model}. Valid options are: {valid_models}")
    ...
    try:
        ...
        # Start diarization
        logging.info(f"Starting diarization for {args.audio_input} using model: {diarization_model}")
        ...
        try:
            ...
            if "Invalid user token" in error_msg or "401" in error_msg:
                ...
                logging.error("3. Try again with the --verify-auth flag to test your authentication")
            ...
            elif "license" in error_msg.lower() or "403" in error_msg or "access" in error_msg.lower():
                model_id = diarization_manager.get_model_id()
                ...
                logging.error("Accept the user conditions on both pages while logged in to your account.")
                sys.exit(1)
        ...
        if args.transcribe:
            ...
            for i, segment in enumerate(segments):
                ...
                cmd = [
                    ...
                    '--audio-input', segment_file,
                    '--model-id', args.model_id,
                    '--backend', args.backend,
                    '--device', args.device or 'auto',
                    '--language', language_arg  # Always specify language to skip detection
                ]
